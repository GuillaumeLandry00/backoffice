...
function sqlModifierClient($conn, $id, $client_nom, $client_prenom, $client_adresse, $client_telephone) {
    ...
    $stmt = mysqli_prepare($conn, $req);
    mysqli_stmt_bind_param($stmt, "sssss", $client_nom, $client_prenom, $client_adresse, $client_telephone,$id);
   if (mysqli_stmt_execute($stmt)) {
        return mysqli_stmt_affected_rows($stmt);
    } else {
        errSQL($conn);
...
function sqlAjoutClient($conn, $client_nom, $client_prenom, $client_adresse, $client_telephone) {
    ...
    VALUES (?, ?, ?, ?)";
    $stmt = mysqli_prepare($conn, $req);
    mysqli_stmt_bind_param($stmt, "ssss", $client_nom, $client_prenom, $client_adresse, $client_telephone);
   if (mysqli_stmt_execute($stmt)) {
        return mysqli_stmt_affected_rows($stmt);
    } else {
        errSQL($conn);
...
* 0 si aucune modification
*/
function sqlSuppressionClient($conn, $id) {

    $req = "DELETE FROM clients WHERE client_id =?";
...
function sqlModifierCategorie($conn, $id, $categorie_nom) {
    ...
    $stmt = mysqli_prepare($conn, $req);
        mysqli_stmt_bind_param($stmt, "ss", $categorie_nom, $id);
    if (mysqli_stmt_execute($stmt)) {
...
/**
* Fonction sqlSuppressionCategorie
* Auteur : Guillaume
* Date :
* But : modifier une ligne dans la table client
* Arguments en entrée : $conn = contexte de connexion
...
* 0 si aucune modification
*/
function sqlSuppressionCategorie($conn, $id) {

    $req = "DELETE FROM categories WHERE categorie_id =?";
 ...
 *                               tableau vide si non trouvée     
 */
function sqlLireCategorieFk($conn, $id) {

    $req = "SELECT 
            produit_id,produit_fk_categories
            FROM produits
            WHERE produit_fk_categories=?";
...
function sqlModifierCommande($conn, $id, $commande_etat, $commande_adresse, $commande_fk_client, $commande_desc="") {
    ...
    $stmt = mysqli_prepare($conn, $req);
    mysqli_stmt_bind_param($stmt, "sssss", $commande_etat, $commande_adresse, $commande_fk_client, $commande_desc,$id);
   if (mysqli_stmt_execute($stmt)) {
        return mysqli_stmt_affected_rows($stmt);
    } else {
        errSQL($conn);
...
function sqlAjoutUtilisateur($conn, $utilisateur_nom, $utilisateur_type, $utilisateur_mot_passe) {
    ...
    VALUES (?,?,SHA2(?, 256))";
   $stmt = mysqli_prepare($conn, $req);
    mysqli_stmt_bind_param($stmt, "sss", $utilisateur_nom, $utilisateur_type, $utilisateur_mot_passe);
   if (mysqli_stmt_execute($stmt)) {
        return mysqli_stmt_affected_rows($stmt);
    } else {
        errSQL($conn);
...
function sqlSuppressionUtilisateur($conn, $id) {
    ...
    $req = "DELETE FROM utilisateurs WHERE utilisateur_id =?";
   $stmt = mysqli_prepare($conn, $req);
    mysqli_stmt_bind_param($stmt, "s", $id);
   if (mysqli_stmt_execute($stmt)) {
        return mysqli_stmt_affected_rows($stmt);
...
function sqlModifierUtilisateur($conn, $id, $utilisateur_nom, $utilisateur_type, $utilisateur_mot_passe) {
    ...
    $req = "UPDATE utilisateurs 
            ...
            utilisateur_nom=?,
            utilisateur_type=?,
            utilisateur_mot_passe=SHA2(?, 256)
            WHERE utilisateur_id = ?";

    $stmt = mysqli_prepare($conn, $req);
    mysqli_stmt_bind_param($stmt, "ssss",$utilisateur_nom,$utilisateur_type,$utilisateur_mot_passe, $id);
   if (mysqli_stmt_execute($stmt)) {
        return mysqli_stmt_affected_rows($stmt);
    } else {
        errSQL($conn);
